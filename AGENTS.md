...
Core lifecycle: `ask()` pops the next question from `deck`, renders shuffled choice buttons, opens the `<dialog id="question">`, and calls `read()`. `answer(choice, button)` scores the pick, disables the choices, and either lets a correct answer auto-advance or shows a "try again"/"next" button for an incorrect one (see **Narration and pacing**). `goToNext()` (assigned to `$("next").onclick` and also called by the auto-advance timer) closes the dialog and routes to `driveToNextStop()` (or the boarding animation before it) or back into `ask()` for a retry.
...
On a **correct** answer, `answer()` hides the next/continue button entirely and auto-advances via `goToNext()` ~2 seconds after the praise line finishes speaking, with a 4-second safety timeout in case the speech `onend`/`onerror` events never fire (a known unreliability on iOS Safari) — the game must never stall waiting on a speech event alone. On an **incorrect** answer (and not yet at 3 wrong), the next button stays visible and focused, requiring a tap, so the child can see the correct answer before moving on.
...
- Correct answers auto-advance without a button tap (see **Narration and pacing**); incorrect answers still require one.
